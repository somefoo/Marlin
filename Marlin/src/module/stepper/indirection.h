...
// X2 Stepper
  ...
  #define X2_STEP_INIT() SET_OUTPUT(X2_STEP_PIN)
    ...
    #define X2_STEP_WRITE(STATE) do {uint8_t wrte = 0; static int32_t x2 = 0; if (X2_DIR_READ()) x2 += X2_STEPS_SCALE; else x2 -= X2_STEPS_SCALE; if (x2 >= X2_STEPS_BASE) { x2 -= X2_STEPS_BASE; wrte = 1; } if (x2 <= -X2_STEPS_BASE) { x2 += X2_STEPS_BASE; wrte = 1; } if (wrte) WRITE(X2_STEP_PIN,STATE); } while(0)
  #endif
